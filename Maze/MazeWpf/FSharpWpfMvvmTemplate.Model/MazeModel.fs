...
module MazeModel =
    ...
    let empty = { environment = AntiObject.emptyEnvironment; maze = []; w = 20; h = 20; wallSize = 20.0; coinX = 0.0; coinY = 0.0; update = id }

    let inline mapRooms mazeEnv =
        let mkWall x y =
            (List.zip [(-1,0); (0,-1); (1,0); (0, 1)] [V(x-1,y); H(x,y-1); V(x,y); H(x,y)]) 
            |>List.filter (not << flip List.exists mazeEnv.maze << (=) <<snd)
            |>List.map fst
        Array2D.init (mazeEnv.w + 1) (mazeEnv.h + 1) mkWall
    ...
    let mazeToPath w h mazeEnv =
        ...
        let iter wall  =
            ...
            | H(x, y) ->
                ...
                builder.Append(sprintf "M%f,%f" xf (yf + mazeEnv.wallSize))|>ignore
                builder.Append(sprintf "H%f"   (xf + mazeEnv.wallSize))|>ignore
            | V(x, y) ->
                let xf, yf =(float x) * mazeEnv.wallSize, (float y) * mazeEnv.wallSize
                builder.Append(sprintf "M%f,%f" (xf + mazeEnv.wallSize) yf)|>ignore
                builder.Append(sprintf "V%f" (yf + mazeEnv.wallSize))|>ignore
